...
class DeferFee extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
    ...
    toSearch = (paramsObj) => {    
        const { symbolid,start, end, acId } = this.state;
        paramsObj={
            accountid: acId,
            symbolid: symbolid,
            start_time: start,
        ...
        paramsObj.limit = 10;
        this.props.dispatch({ type: 'deliverDeclare/queryDiYanFee', payload: paramsObj });
    }
    render() {
        const {
            acSymbolsObj,
