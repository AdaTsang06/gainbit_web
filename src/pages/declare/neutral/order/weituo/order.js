...
import { dir,deliver_status_reverse,deliver_status } from '../../../../../common/constant-enum';
...
const getOpenOrdersColumn = (cancelOrder) => {
    ...
    },
    {
      ...
      align: 'right',
      render: (value,item) => {
        const init = item.status === deliver_status.deliver_init;
        return (
        <div>
          <a
              style={{          
...
@connect((state) => ({
    history: state.neutralDeclare.weituoHistory.data,
    count: state.neutralDeclare.weituoHistory.count,
